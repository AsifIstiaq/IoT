...
// =====================================================================
//  motor_control.h — L293D PWM + direction driver.
//  Same signed-speed interface the Uno version used (setMotors(left,
//  right), positive = forward, negative = reverse) so the rest of the
//  logic (line PID, turns, recovery) ports over unchanged. The only
//  difference is ESP32 uses the LEDC peripheral instead of analogWrite().
// =====================================================================
...
// Plain digitalWrite beep instead of tone(): on ESP32 core 2.0.x, tone()
// shares the same LEDC hardware as our motor PWM channels and can glitch
// or reset them. A simple HIGH/LOW pulse works fine with the active
// buzzer modules typically bundled in this kind of kit.
